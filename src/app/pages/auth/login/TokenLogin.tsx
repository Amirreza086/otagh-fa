      ...
      {loginState.status === AsyncStatus.Error && (
        <>
          {loginState.error.errcode === LoginError.Forbidden && (
            <LoginTokenError message="رمز ورود نامعتبر است." />
          )}
          {loginState.error.errcode === LoginError.UserDeactivated && (
            <LoginTokenError message="این حساب غیر فعال شده است." />
          )}
          {loginState.error.errcode === LoginError.InvalidRequest && (
            <LoginTokenError message="ورود ناموفق بود. بخشی از اطلاعات درخواست شما نامعتبر است." />
          )}
          {loginState.error.errcode === LoginError.RateLimited && (
            <LoginTokenError message="ورود ناموفق بود. درخواست ورود شما توسط سرور با نرخ محدود شده است، لطفاً بعد از مدتی امتحان کنید." />
          )}
          {loginState.error.errcode === LoginError.Unknown && (
            <LoginTokenError message="ورود ناموفق بود. به دلیل نامعلوم." />
          )}
        </>
      )}
